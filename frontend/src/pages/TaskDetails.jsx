...
function TaskDetails() {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '50px auto', padding: '20px' }}>
      <button onClick={() => navigate('/dashboard')} style={{ marginBottom: '20px', padding: '10px 20px', cursor: 'pointer', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '5px' }}>
        ← Back to Dashboard
      </button>
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '10px', padding: '30px', marginBottom: '20px' }}>
        ...
        <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f8f9fa', borderRadius: '10px' }}>
          ...
          <p><strong>📊 Status:</strong>
            <span style={{ marginLeft: '10px', padding: '3px 10px', borderRadius: '5px', backgroundColor: task.status === 'APPROVED' ? '#d4edda' : task.status === 'SUBMITTED' ? '#fff3cd' : '#f8d7da', color: task.status === 'APPROVED' ? '#155724' : task.status === 'SUBMITTED' ? '#856404' : '#721c24' }}>
              {task.status}
            </span>
          </p>
        ...
        {task.proof && (
          <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#e7f3ff', borderRadius: '10px' }}>
            ...
            {task.proof.type === 'image' && <img src={task.proof.url} alt="Proof" style={{ maxWidth: '100%', maxHeight: '300px', borderRadius: '10px' }} />}
            {task.proof.type === 'link' && <a href={task.proof.url} target="_blank" rel="noopener noreferrer" style={{ color: '#667eea' }}>{task.proof.url}</a>}
        ...
        {canSubmit && (
          <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#f8f9fa', borderRadius: '10px' }}>
            ...
            <form onSubmit={handleSubmitProof}>
              ...
              {proofType === 'link'
                ? <input type="url" placeholder="https://example.com/proof" value={proofLink} onChange={e => setProofLink(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #ddd', boxSizing: 'border-box' }} required />
                : <input type="file" onChange={e => setProofFile(e.target.files[0])} style={{ width: '100%', padding: '10px', marginBottom: '10px' }} required />
              }
              <textarea placeholder="Add a caption or notes (optional)" value={caption} onChange={e => setCaption(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #ddd', minHeight: '80px', boxSizing: 'border-box' }} rows="2" />
              <button type="submit" style={{ width: '100%', padding: '12px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>Submit Proof</button>
            </form>
          </div>
        )}
        ...
        {canApprove && (
          <div style={{ marginTop: '20px', display: 'flex', gap: '10px' }}>
            <button onClick={() => handleApprove(true)} style={{ flex: 1, padding: '12px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>✅ Approve Task</button>
            <button onClick={() => { const reason = prompt('Reason for rejection:'); if (reason) handleApprove(false, reason) }} style={{ flex: 1, padding: '12px', backgroundColor: '#dc3545', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>❌ Reject Task</button>
          </div>
        )}
      </div>
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '10px', padding: '30px' }}>
        ...
        <form onSubmit={handleAddComment} style={{ display: 'flex', gap: '10px' }}>
          <input type="text" placeholder="Type your message..." value={newComment} onChange={e => setNewComment(e.target.value)} style={{ flex: 1, padding: '12px', borderRadius: '5px', border: '1px solid #ddd' }} required />
          <button type="submit" style={{ padding: '12px 24px', backgroundColor: '#667eea', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>Send</button>
        </form>
      </div>
    </div>
  )
